...
function StatCard({ title, value, subtitle, icon: Icon, trend, trendType }) {
  return (
...
function AcademiaDashboard() {
  ...
  const [institutionalPriority, setInstitutionalPriority] =
  useState(null);
  ...
  collaborationsData,
          ...
          apiGet("/academia/institutional-priority"),
...
setOpportunities(
  opportunitiesData || []
        ...
        setError(err.message || "Failed to load academia dashboard.");
      } finally {
        setLoading(false);
      }
    }
  ...
  const totalStudents = dashboard?.stats?.total_students || 0;

  const averageReadiness = dashboard?.stats?.average_readiness || 0;

  const activeInternships = placement?.summary?.active_opportunities || 0;

  const placementRate = placement?.summary?.placement_rate || 0;
  ...
  const criticalSkillCount = skillGaps?.summary?.critical_skills || 0;

  const studentsAffected = skillGaps?.summary?.students_affected || 0;
  ...
  const displayedDemand = Array.isArray(skillDemand)
  ...
  const displayedGaps = Array.isArray(skillGaps)
    ? skillGaps.slice(0, 4)
  ...
  /*
 * Show the highest-priority institutional skills.
 */

const displayedPrioritySkills =
  Array.isArray(institutionalPriority?.skills)
    ? institutionalPriority.skills.slice(0, 4)
    : [];
  ...
  const recentActivities = [
    ...opportunities.slice(0, 3).map((opportunity) => ({
      ...
      detail: `${opportunity.type} opportunity opened`,
      date: opportunity.title,
      icon: "opportunity",
    })),
    ...
    <div className="space-y-7">
      ...
      <div>
        ...
        <p className="text-slate-500 mt-2">
          Monitor student skills, industry demand and institutional readiness.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-4">
        ...
        <StatCard
          ...
          value={criticalSkillCount}
          subtitle={`${studentsAffected} students affected`}
          icon={AlertTriangle}
        />
        ...
        <section className="bg-white border border-slate-200 rounded-2xl p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500 mt-1">
                Department-level analytics will appear once department data is
                available.
              </p>
            </div>
          ...
          <div className="mt-8 border border-dashed rounded-xl p-8 text-center">
            ...
            <p className="text-xs text-slate-400 mt-1">
              The current Student model does not yet store department
              information.
            </p>
          </div>
        </section>
                ...
                <div key={item.id}>
                  ...
                  <div className="flex items-center justify-between gap-3 mb-2">
                    ...
                    <div className="flex items-center gap-2 text-xs">
                      ...
                      >
                        {item.gap > 0 ? `Gap ${item.gap}%` : "Balanced"}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex justify-end mb-2">
                    {item.gap >= 30 ? (
                      <span className="text-[11px] font-medium text-red-600 bg-red-50 px-2 py-1 rounded-full whitespace-nowrap">
                        Critical Shortage
                      </span>
                    ) : item.gap >= 10 ? (
                      <span className="text-[11px] font-medium text-orange-600 bg-orange-50 px-2 py-1 rounded-full whitespace-nowrap">
                        Shortage
                      </span>
                    ) : (
                      <span className="text-[11px] font-medium text-green-600 bg-green-50 px-2 py-1 rounded-full whitespace-nowrap">
                        Balanced
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex justify-between mt-1.5 text-[11px] text-slate-400">
                    <span>
                      {item.students_meeting_target || 0} students meeting
                      target
                    </span>

                    <span>
                      {item.opportunity_count || 0} active opportunit
                      {item.opportunity_count === 1 ? "y" : "ies"}
                    </span>
                  </div>
                </div>
              ))
      ...
      {/* Institutional Priority Intelligence */}
...
<section className="bg-white border border-slate-200 rounded-2xl p-6">
  ...
  <div className="flex items-center justify-between">
    ...
    <div>
      ...
      <h2 className="text-lg font-semibold text-slate-900">
        Institutional Priority Skills
      </h2>

      <p className="text-sm text-slate-500 mt-1">
        Skills where student readiness and industry demand require attention
      </p>
  ...
  <div className="mt-6">
    ...
    {displayedPrioritySkills.length === 0 ? (

      <p className="text-sm text-slate-400">
        No institutional priority data available.
      </p>
      ...
      <div className="space-y-4">
        ...
        {displayedPrioritySkills.map((item) => (
          ...
          >
            ...
            <div className="flex items-center justify-between gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">
                  Average readiness {item.average_score}%
                  {" · "}
                  Industry demand {item.industry_demand}%
                </p>
            ...
            <div className="flex items-center justify-between mt-3">
              ...
              <span className="text-xs text-slate-400">
                {item.opportunity_count || 0} industry opportunit
                {item.opportunity_count === 1 ? "y" : "ies"}
              </span>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-100 text-left">
                    <th className="pb-3 font-medium text-slate-500">Skill</th>
        ...
        <section className="bg-white border border-slate-200 rounded-2xl p-6">
          ...
          <button
            onClick={() => (window.location.href = "/academia/opportunities")}
