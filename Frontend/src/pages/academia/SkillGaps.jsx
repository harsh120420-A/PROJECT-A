...
function SkillGaps() {
  ...
  async function loadSkillGaps() {
    try {
      ...
      const [gapResponse, demandResponse] = await Promise.all([
      ...
      setIndustryDemand(demandResponse?.skills || []);
    } catch (err) {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <p className="text-slate-500">Loading skill gap analytics...</p>
      </div>
    );
  }
  ...
  const industryRelevantGaps = gaps
    .map((skillGap) => {
      const demand = industryDemand.find((item) => item.id === skillGap.id);

      return {
        ...skillGap,

        industry_demand: demand?.demand || 0,

        student_supply: demand?.supply || 0,

        demand_gap: demand?.gap || 0,

        opportunity_count: demand?.opportunity_count || 0,
      };
    })
    .sort((a, b) => {
      const priorityScoreA = a.gap * 0.6 + a.industry_demand * 0.4;

      const priorityScoreB = b.gap * 0.6 + b.industry_demand * 0.4;

      return priorityScoreB - priorityScoreA;
    });

  const criticalSkills = summary.critical_skills || 0;

  const highPrioritySkills = summary.high_priority_skills || 0;

  const studentsAffected = summary.students_affected || 0;

  const largestGap = summary.largest_gap || 0;

  const largestGapSkill = summary.largest_gap_skill || "None";
  ...
  return (
    <div className="space-y-7">
      ...
      <div>
        ...
        <h1 className="text-3xl font-bold text-slate-900 mt-1">Skill Gaps</h1>

        <p className="text-slate-500 mt-2">
          Identify skill shortages and prioritize institutional development
          initiatives.
        </p>
      </div>
      ...
      <section className="bg-white border border-slate-200 rounded-2xl p-6">
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              Compare current student readiness with the institutional target
              readiness level of 70%.
            </p>
          </div>
        ...
        {gaps.length === 0 ? (
          <div className="py-14 text-center">
            ...
            <p className="text-sm text-slate-500 mt-1">
              All assessed skills currently meet the institutional target.
            </p>
          </div>
        ) : (
              ...
              <div key={item.id}>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                  ...
                  <div className="flex items-center gap-4 text-xs">
                    ...
                    <span className="font-bold text-red-600">{item.gap}%</span>
                  </div>
                </div>
      ...
      <section className="bg-white border border-slate-200 rounded-2xl p-6">
        <div>
          <h2 className="text-lg font-semibold text-slate-900">Gap Overview</h2>
      ...
      <section className="bg-white border border-slate-200 rounded-2xl p-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Skills prioritized using both student readiness gaps and current
            industry demand.
          </p>
        </div>

        {industryRelevantGaps.length === 0 ? (
          <div className="py-10 text-center">
            <CheckCircle2 size={36} className="mx-auto text-green-500" />

            <p className="text-sm text-slate-500 mt-3">
              No industry-relevant skill gaps identified.
            </p>
          </div>
        ) : (
          <div className="mt-6 overflow-x-auto">
            <table className="w-full min-w-[750px]">
              ...
              <tbody>
                {industryRelevantGaps.map((item) => {
                  const priorityScore =
                    item.gap * 0.6 + item.industry_demand * 0.4;
      ...
      <section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mt-5">
          {industryRelevantGaps
  .filter(
    (item) =>
      item.industry_demand > 0
  )
  .slice(0, 3)
  .map((item) => {
            const isLearning =
              item.name === "Cloud Computing" || item.name === "Power BI";
  ...
  Prioritize industry-relevant skill development.
</h2>

              <p className="text-sm text-slate-300 mt-2 max-w-4xl leading-relaxed">

  {industryRelevantGaps.length > 0
    ? `Institutional development should focus on ${
        industryRelevantGaps
          .slice(0, 3)
          .map((item) => item.name)
          .join(", ")
      }. These skills combine student readiness gaps with current industry demand and therefore represent strong candidates for targeted training, practical projects, and industry collaboration.`
    : "No industry-relevant skill gaps have been identified from the current student and opportunity data."}
