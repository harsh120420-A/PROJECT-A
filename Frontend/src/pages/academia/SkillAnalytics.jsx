...
function SkillAnalytics() {
  ...
  useEffect(() => {
    async function loadAnalytics() {
      try {
        ...
        const data = await apiGet("/academia/skill-analytics");
  ...
  const proficiencyData = analytics?.proficiency_distribution || [];

  /*
   * Strongest skills
   */

  const strongestSkills = analytics?.strongest_skills || [];

  const prioritySkills = analytics?.priority_skills || [];
  ...
  summary.technical_readiness || 0;

const softAverage =
  summary.soft_skill_readiness || 0;
    ...
    <div className="space-y-7">
      ...
      <div>
        ...
        <p className="text-slate-500 mt-2">
          Understand institution-wide skill readiness and identify areas
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          value={summary.students_assessed || 0}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <section className="bg-white border border-slate-200 rounded-2xl p-6">
          ...
          <div className="mt-8 space-y-8">
            ...
            <div>
              <div className="flex justify-between mb-2">
                ...
                <span
                  className={`text-sm font-bold ${getScoreColor(softAverage)}`}
                >
                  {softAverage}%
                </span>
              </div>
              ...
              <div className="h-3 bg-slate-100 rounded-full">
                <div
                  className={`h-full rounded-full ${getBarColor(softAverage)}`}
                  style={{
                    width: `${softAverage}%`,
                  }}
                />
          ...
          <div className="mt-8 pt-5 border-t border-slate-100">
            <p className="text-xs text-slate-400">Institutional average</p>
          ...
          <div className="mt-5 space-y-4">
            {prioritySkills.map((item) => (
              ...
              >
                ...
                <p className="text-xs text-slate-500 mt-2">{item.average_score < 40
  ? "Low institutional readiness"
  : item.average_score < 60
  ? "Needs further development"
  : "Requires continued improvement"}</p>
              </div>
            ))}
          </div>
        </section>
