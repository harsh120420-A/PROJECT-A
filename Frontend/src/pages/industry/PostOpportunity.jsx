...
function PostOpportunity() {
  ...
  function addSkill() {
    ...
    const alreadyExists = skills.some(
      (existingSkill) =>
        existingSkill.name.toLowerCase() === matchedSkill.toLowerCase(),
    );
  ...
  async function handleSubmit(e) {
    ...
    try {
      const response = await apiPost("/industry/opportunities", {
        title: form.title,
        type: form.type,
      ...
      setMessage(error.message || "Failed to publish opportunity.");
    }
  }
          ...
          <div className="bg-white border rounded-2xl p-6 mt-6">
            ...
            {skills.length === 0 && (
              <p className="text-sm text-slate-400 mt-4">
                No skills added yet. Add the skills required for this
                opportunity.
              </p>
            )}
          </div>
